...
resource "aws_ecs_task_definition" "samproxy" {
  family = var.name
  ...
  container_definitions = module.samproxy.json_map_encoded_list

  tags = local.tags
}

resource "aws_ecs_service" "samproxy" {
  name    = var.name
  cluster = aws_ecs_cluster.cluster.id

  launch_type     = "FARGATE"
  task_definition = "${aws_ecs_task_definition.samproxy.family}:${aws_ecs_task_definition.samproxy.revision}"
  ...
  network_configuration {
    ...
    security_groups = flatten([
      aws_security_group.samproxy.id,
...
resource "aws_cloudwatch_log_group" "samproxy" {
  name              = var.name
